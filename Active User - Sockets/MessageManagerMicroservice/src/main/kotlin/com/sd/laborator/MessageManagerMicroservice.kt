        ...
        while (true) {
            ...
            thread {
                ...
                while (true) {
                    ...
                    val (messageType, messageDestination) = receivedMessage.split(" ", limit = 3)

                    when (messageType) {
                        "get" -> {
                            if(subscribers.size>1) {
                                print(subscribers.entries
                                    .filter { it.key != messageManagerSocket.localPort && it.key != messageDestination.toInt() }
                                    .joinToString(" ") { (key, value) -> "$key-${value.inetAddress.hostAddress}" })
                                respondTo(
                                    messageDestination.toInt(),
                                    subscribers.entries.filter { it.key != messageManagerSocket.localPort && it.key != messageDestination.toInt() }
                                        .joinToString(" ") { (key, value) -> "$key-${value.inetAddress.hostAddress}" })
                            }
                            else{
                                print("Lista goala")
